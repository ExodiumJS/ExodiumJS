/**********************************************************\
 *                                                        *
 *   _____               _ _                     _ ____   *
 *  | ____|_  _____   __| (_)_   _ _ __ ___     | / ___|  *
 *  |  _| \ \/ / _ \ / _` | | | | | '_ ` _ \ _  | \___ \  *
 *  | |___ >  < (_) | (_| | | |_| | | | | | | |_| |___) | *
 *  |_____/_/\_\___/ \__,_|_|\__,_|_| |_| |_|\___/|____/  *
 *                                                        *
 *          This file is licensed under the GNU           *
 *          General Public License 3. To use or           *
 *          modify it you must accept the terms           *
 *          of the license.                               *
 *              ___________________________               *
 *              \ @author ExodiumJS Team /                *
 \**********************************************************/

const DataPacket = require("./DataPacket");
const ProtocolInfo = require("./ProtocolInfo");

class RequestNetworkSettingsPacket extends DataPacket{
    static NETWORK_ID = ProtocolInfo.REQUEST_NETWORK_SETTINGS_PACKET;

    /**
     * @type {number}
     */
    protocolVersion;

    decodePayload() {
        this.protocolVersion = this.readUnsignedIntBE();
    }

    encodePayload() {
        this.writeUnsignedIntBE(this.protocolVersion);
    }

    isCompatibleProtocol(protocolVersion){
        return protocolVersion === ProtocolInfo.CURRENT_PROTOCOL;
    }

    handle(handle) {
        return handle.handleRequestNetworkSettings(this);
    }
}

module.exports = RequestNetworkSettingsPacket;